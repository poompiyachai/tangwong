...
import com.example.krisorn.tangwong.status.StatusAlertAdapter;
...
public class StatusAlert extends AppCompatActivity {
    ...
    private void loadListItem(){
        ...
        mAuth = FirebaseAuth.getInstance();
        Log.d("list1", String.valueOf(countNoti));
        try {
            mDatabase.child("user").child(user.getUid()).child("keep_noti").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    countNoti= (int) dataSnapshot.getChildrenCount();
                    Log.d("list2", String.valueOf(countNoti));
                    adapter = new StatusAlertAdapter(countNoti,StatusAlert.this);
        ...
        Log.d("list", String.valueOf(countNoti));
